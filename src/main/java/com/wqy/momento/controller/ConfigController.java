...
public class ConfigController{
    ...
    @GetMapping("{id}")
    public ResponseEntity<Config> queryById(String id){
        return ResponseEntity.ok(configService.queryById(id));
    }
    ...
    @ApiOperation("分页查询")
    @GetMapping
    public ResponseEntity<PageImpl<Config>> paginQuery(Config config, PageRequest pageRequest){
        //1.分页参数
    ...
    public ResponseEntity<Config> add(Config config){
        return ResponseEntity.ok(configService.insert(config));
    }
    ...
    public ResponseEntity<Boolean> deleteById(String id){
        return ResponseEntity.ok(configService.deleteById(id));
    }
}